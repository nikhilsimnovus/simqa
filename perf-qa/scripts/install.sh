...
UPDATE_TARBALL_URL_DEFAULT="https://github.com/nikhilsimnovus/oneclick/archive/refs/heads/main.tar.gz"
UPDATER_PATH="/usr/local/sbin/perfqa-update"
log "Installing self-update helper -> ${UPDATER_PATH}"
cat > "${UPDATER_PATH}" <<UPDATER
#!/bin/bash
# Auto-generated by perf-qa install.sh. Triggered by the Update icon in
# the Flask UI (POST /api/update). Downloads the latest perf-qa tarball
# from GitHub and re-runs scripts/install.sh from it.
set -euo pipefail
...
curl -fsSL "\$TARBALL_URL" -o "\$TD/main.tar.gz"
